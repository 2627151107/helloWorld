...
(function(){
  ...
  function handle() {
    //alert($("#name").val()+"@"+$("#pass input").val()+"@"+$("#code").val())
    $.ajax({
      url:"customer_register",    //请求的url地址
      data:"customerPhone="+$("#name").val()+"&customerPassword="+$("#pass input").val()+"&dpassword="+$("#code").val(),    //参数值
      type:"post",   //请求方式
      success:function(s){
        if(s=="ok"){
